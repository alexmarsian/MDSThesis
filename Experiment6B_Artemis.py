# Runs CIFAR100 with 20% noise for 9 different sparsity levels
    ...
    for r in range(repeats):
        ...
        for epoch in range(epochs):
            ...
            if val_loss < best_val_loss:
                ...
                torch.save(model.state_dict(), weightsDir / Path(f'best_forward_exp6B.pkl'))
        ...
        model.load_state_dict(torch.load(weightsDir / Path(f'best_forward_exp6B.pkl'), map_location=device))
...
batch_size=128
datapath=datapath
noise_file = "20SymNoiseCifar100"
sparsity = [0.4, 0.3, 0.2, 0.1]
repeats = 3

for s in sparsity:
    sname = int(s*100)
    weightFileName = f"R34_sparse{sname}_Cifar100_{int(noise_rate*100)}pct_{noise_mode}"
